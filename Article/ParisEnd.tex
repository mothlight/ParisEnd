...
Attempts to find quantitative methods, creating city typologies, to assess, describe the major characteristics of the city, and classify different types of urban form have been underway for a number of decades. First attempts used broad demographics and functional characteristics to classify different types of cities. Occupational and employment figures were used to determine a city's most important economic activity (including manufacturing, retail, diversified, wholesale, transportation, mining, education, and resorts) \cite{Harris1943}. Other studies used economic activity data to classify American cities into broad functional typologies such as manufacturing, retail, professional services, and financial services \cite{Nelson1955}. \cite{Bruce1971} performed a cluster analysis based on the socio-economic profiles of the cities as well as a number of census based statistics about the cities to group a number of American cities into clusters. However, in these studies, the typologies are more functional in nature, making the contribution of urban design difficult to examine.

New techniques start to emerge in the 1980s and 90s with the growing availability of databases of spatial data and increased computing power. Much of this recent work focuses on network analysis of the road networks in cities. Drawing from structural sociology, representing groups of people as a network structure, where individuals are nodes and relationships are edges, the road network structure allows typologies to be derived from an analysis of the structure \cite{Porta2006a}, the 'space syntax' of Hillier (1996) \cite{Hillier1996}. These  the properties of a node, the centrality (in relationship to other nodes), closeness (path length to other nodes), and degree (number of connections) 

Through the network analysis of roads, often the most long lasting part of an urban area, much information about the area, its governance, and evolution can be determined. a historical evolution of the urban area can be seen through the growth of road networks, the densification of existing centres and exploration of urban boundaries \cite{Strano2012}. In addition, the road structure can point to the governance systems underlying each urban area, with areas dominated by a grid structure and four-way crossings reflecting a strong system of top-down planning \cite{Courtat2011} while areas with more dead-ends and T-shaped crossing showing more organic growth \cite{Cardillo2006}. Older cities are often more disorderly \cite{Jacobs1961}, as the result of self organised organic growth instead of top down planning. Methods of subdividing land reflect the land uses. Large land blocks (often agricultural land) can follow a progression to smaller shapes either to medium sized manufacturing or smaller residential plots. In addition to this, \cite{Strano2012} also finds that grids systems that are subdivided into smaller grids often result where these areas are unconstrained by adjoining villages or topography. 

Other remote sensing based methods depart from the pure network analysis methods. \cite{Zhang2013} uses remote sensing night-time light data to categorise cities into stages of urbanisation and levels of economic activities. \cite{Hermosilla2014} uses remote sensed urban metrics (road geometry, building dimensions and heights, and vegetation heights) to classify cities into typologies of differing periods of historical design and urban planning (i.e. 19th Century, 1950s, 1970, etc.).
...
%
%
%\cite{Zhang2013} uses remote sensing night-time light data to categorise cities into stages of urbanisation and levels of economic activities.
%
%\cite{Hermosilla2014} uses remote sensed urban metrics (road geometry, building dimensions and heights, and vegetation heights) to classify cities into typologies of differing periods of historical design and urban planning (i.e. 19th Century, 1950s, 1970, etc.).
%
...
%the network approach to cities and streets ('space syntax' of Hillier 1996 \cite{Hillier1996} )  can be captured in closeness centrality.
...
However, all these methods requite some amount of subjective classification. In addition, much of the data required is difficult to acquire and can vary widely across different political jurisdictions. To overcome these limitations, we develop a new framework using an entirely new and highly efficient `bottom-up` approach. Utilising recent advances in computing power and artificial intelligence, as well as an increased availability of globally consistent urban imagery, we train three neural networks using millions of images of urban form (consisting of street view, satellite imagery, and street maps) from over 1500 world-wide cities. 
...
%\todo[inline]{
%Should I look at how similar cities are grouped according to age of the city, founding dates?
%}
...
\todo[inline]{
Say that this is where you see some of the urban form but also the way the urban form is being used (cars, street furniture, etc)
}
